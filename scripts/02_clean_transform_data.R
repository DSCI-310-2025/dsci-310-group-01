...
longbeach_cleaned <- longbeach %>%
  ...
  filter(as.Date(dob) <= as.Date("2024-12-31")) %>% # Remove future dates
  select(-animal_name, -secondary_color) # Drop unimportant columns with many missing values
...
# # Remove ages  > 30
# longbeach_cleaned <- longbeach_cleaned %>% filter(age <= 30)
