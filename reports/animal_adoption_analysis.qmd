...
library(tidyverse)
library(animalAdoptR)  # Now using our custom package for all core analysis functions
...
Below are some of the key variables from the original dataset that are relevant to predicting adoption outcomes (after removing variables with limited analytical value):
...
rescue_match <- outcome_df$count[which(tolower(outcome_df$outcome_type) == "rescue")]
rescue_count <- na.omit(rescue_match)[1]
adoption_match <- outcome_df$count[which(tolower(outcome_df$outcome_type) == "adoption")]
adoption_count <- na.omit(adoption_match)[1]
...
knitr::kable(head(outcome_df, 12),
             ...
             row.names = FALSE)
```
"Rescue" is the most common outcome (`r format(rescue_count, big.mark = ",")` animals), 
followed closely by "adoption" (`r format(adoption_count, big.mark = ",")` animals).
...
We selected a Random Forest model due to its robustness with categorical features and ability to handle non-linear relationships. The model was trained using the following approach:

1. **Train-Test Split**: The dataset was split into 80% training and 20% testing sets using stratified sampling to maintain the same class distribution in both sets.
2. **Class Balance**: As showns in @fig-adoption-distribution, with only `r round(100 - non_adopted_percentage, 3)`% of animals being adopted. To prevent the model from biasing toward the majority class, we applied downsampling to the training data:
...
3. **Model Parameters**: We used 100 trees (`ntree=100`) and 2 variables sampled at each split (`mtry=2`)

#### Model Performance Evaluation

```{r}
model_metrics <- read_csv("../results/tables/metrics.csv")
accuracy <- round(model_metrics$Accuracy * 100, 1)
sensitivity <- round(model_metrics$Sensitivity * 100, 1)
specificity <- round(model_metrics$Specificity * 100, 1)
```

![Confusion matrix showing prediction results for animal adoption](../results/figures/confusion_matrix.png){#fig-confusion-matrix width=75%}

The confusion matrix (@fig-confusion-matrix) visualizes our model's performance on the test dataset. Our Random Forest classifier achieved an overall accuracy of `r accuracy`%. The model demonstrates a sensitivity (true positive rate) of `r sensitivity`%, indicating its ability to correctly identify animals that were actually adopted. The specificity (true negative rate) of `r specificity`% shows the model's effectiveness in identifying animals that were not adopted.

```{r}
#| echo: false
# Load confusion matrix summary directly from saved CSV
conf_matrix_summary <- read_csv("../results/tables/confusion_matrix_summary.csv")

# Extract values from the CSV
true_positive <- conf_matrix_summary$Count[conf_matrix_summary$Metric == "True Positives"]
false_negative <- conf_matrix_summary$Count[conf_matrix_summary$Metric == "False Negatives"]
false_positive <- conf_matrix_summary$Count[conf_matrix_summary$Metric == "False Positives"]
true_negative <- conf_matrix_summary$Count[conf_matrix_summary$Metric == "True Negatives"]
```

Looking at the raw counts in the confusion matrix, we can see that out of animals actually adopted (Yes), the model correctly identified `r true_positive` cases while misclassifying `r false_negative` as not adopted. For animals that were not adopted (No), the model correctly identified `r true_negative` cases while misclassifying `r false_positive` as adopted. These results suggest the model performs slightly better at identifying non-adopted animals, which aligns with the class distribution in our original dataset.

#### Feature Importance

![Feature importance for the Random Forest model](../results/figures/feature_importance.png){#fig-feature-importance width=70%}

@fig-feature-importance reveals the relative importance of each feature in predicting adoption outcomes. The most influential factors in determining whether an animal will be adopted are:

1. Sex - The animal's sex and reproductive status (neutered/spayed) emerged as the most important predictor.
2. Age - Younger animals show significantly higher adoption rates.
3. Intake type - How an animal arrived at the shelter (stray, owner surrender, etc.) substantially affects adoption outcomes.
4. Animal type - Different species have varying adoption rates.
5. Season - Time of year shows a modest effect on adoption probability.
6. Intake condition - The animal's health status when entering the shelter influences adoption likelihood.

This feature ranking provides valuable insights for shelter management, suggesting that factors like **sex** and **age** are particularly important considerations when developing targeted adoption strategies.


### Discussion

Our analysis of the Long Beach Animal Shelter data revealed that sex/reproductive status, age, and intake type are the most influential factors in animal adoption outcomes. The strong influence of sex suggests that spay/neuter status significantly impacts adoption decisions, while the consistent preference for younger animals presents an ongoing challenge for shelters placing older pets. Additionally, the manner in which animals enter the shelter affects their adoption prospects, with owner-surrendered animals generally finding homes more easily than strays or confiscated animals.


## Conclusion

This study successfully developed a predictive model for animal adoption outcomes at the Long Beach Animal Shelter. Our Random Forest classifier identified key factors influencing adoption, with the model achieving an accuracy of `r accuracy`%. The analysis demonstrated that animal sex/reproductive status, age, and intake type are the strongest predictors of whether an animal will be adopted, providing valuable insights for shelter management decision-making.

These findings contribute to the growing body of research on animal welfare and shelter management, providing data-driven insights to support evidence-based practices in animal sheltering and adoption.
